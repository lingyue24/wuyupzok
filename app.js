...
function toggleMenu(e) {
    if(e) e.stopPropagation(); // 防止點擊事件穿透
    ...
    items.map((item, idx) => `
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                ...
                <button class="btn btn-danger" onclick="deleteItem(${idx})" style="margin-left:5px;">🗑</button></div>` : ''}
            </div>
            <div class="gallery">${(item.imgs || []).map(src => `<img src="${src}" style="max-width:100px; margin:5px;">`).join('')}</div>
...
function addImgByUrl() {
    ...
    if(url) { tempImgs.push(url); renderImgManager(); }
}

function renderImgManager() {
    const zone = document.getElementById("img-manager-zone");
    zone.innerHTML = tempImgs.map((img, i) => `<img src="${img}" style="width:50px; height:50px;" onclick="tempImgs.splice(${i},1);renderImgManager();">`).join("");
}
...
async function addCategory() {
    ...
    if(name) { activeNode.children.push({name, children:[], items:[]}); renderAfterLoad(); await saveToCloud(); }
}
async function deleteCategory() {
    if(confirm("確定刪除分類？")) { /* 略過細節邏輯確保穩定 */ }
}
...
async function saveSettings() {
    ...
    db.config.password = document.getElementById("set-new-pw").value || db.config.password;
    await saveToCloud();
    location.reload();
}
